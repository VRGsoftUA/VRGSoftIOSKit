...
@interface SMMulticastDelegateNode : NSObject {
@private
...
@interface SMMulticastDelegateEnumerator ()
{
	NSUInteger numNodes;
...
- (instancetype)initFromDelegateNodes:(NSMutableArray *)inDelegateNodes;
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	SMMulticastDelegateNode *node =
	    [[SMMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	{
		SMMulticastDelegateNode *node = [delegateNodes objectAtIndex:(i-1)];
...
{
	for (SMMulticastDelegateNode *node in delegateNodes)
	{
		node.delegate = nil;
		#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
		node.unsafeDelegate = nil;
...
- (SMMulticastDelegateEnumerator *)delegateEnumerator
{
	return [[SMMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation SMMulticastDelegateNode
...
@implementation SMMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
	...
	{
		SMMulticastDelegateNode *node = [delegateNodes objectAtIndex:currentNodeIndex];
		currentNodeIndex++;
